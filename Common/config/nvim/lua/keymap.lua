...
vim.keymap.set('n', '<S-F1>', function() vim.diagnostic.open_float({ focusable = false }) end)
vim.keymap.set('n', '<F2>',
  function()
    vim.diagnostic.jump({
      diagnostic = vim.diagnostic.get_next(),
      on_jump = function() vim.diagnostic.open_float({ focusable = false }) end
    })
  end
)
vim.keymap.set('n', '<S-F2>',
  function()
    vim.diagnostic.jump({
